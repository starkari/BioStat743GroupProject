...
death.fit1 <- glm(death ~ AGE + as.factor(SEX) + as.factor(IBS_POST) + as.factor(DLIT_AG) + as.factor(SIM_GIPERT) + as.factor(endocr_01) + as.factor(endocr_02) + AGE * IBS_POST + AGE * DLIT_AG + AGE * SIM_GIPERT + AGE * endocr_01 + AGE * endocr_02, data = data.work2, family = binomial)
step(death.fit1, death.fit0, direction = "both") # selected variable: AGE, IBS_POST, DLIT_AG, SIM_GIPERT, endocr_01, endocr_02
# fit the best model with different link function
death.fit.logit <- glm(death ~ AGE + as.factor(IBS_POST) + as.factor(DLIT_AG) + as.factor(SIM_GIPERT) + as.factor(endocr_01) + as.factor(endocr_02), data = data.work2, family = binomial)
deviance(death.fit.logit)

death.fit.probit <- glm(death ~ AGE + as.factor(IBS_POST) + as.factor(DLIT_AG) + as.factor(SIM_GIPERT) + as.factor(endocr_01) + as.factor(endocr_02), data = data.work2, family = binomial(link = probit))
deviance(death.fit.probit)

death.fit.cloglog <- glm(death ~ AGE + as.factor(IBS_POST) + as.factor(DLIT_AG) + as.factor(SIM_GIPERT) + as.factor(endocr_01) + as.factor(endocr_02), data = data.work2, family = binomial(link = cloglog))
deviance(death.fit.cloglog)
...
glm.predict <- ifelse(predict(death.fit, data.work2, type = "response") > 0.5, 1, 0)
sum(diag(table(glm.predict, data.work2$ZSN))) / nrow(data.work2)
