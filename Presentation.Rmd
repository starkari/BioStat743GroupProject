...
- Test independence of variables in $2\times 2$ contingency tables

- Fit a logistic linear regression model with a predictor and Chronic heart failure

- Build a multiple logistic regression model by adding more predictors and identify the best model

- Build a model to predict Chronic heart failure related death and check the model prediction accuracy
...
```{r}
kable(summary(death.mod)$coefficients)
```
